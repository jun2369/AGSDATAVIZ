...
const DeliveryKPI: React.FC<DeliveryKPIProps> = ({ uploadedData }) => {
  ...
  const processExcelData = (data: any[]) => {
    ...
    for (let i = 2; i < data.length; i++) {
      ...
      const handoverTime = parseExcelDate(handoverTimeValue);

      // Skip if either date is missing or invalid
      if (!consignedDate || !handoverTime) {
        skipCount++;
        if (i < 10) {
          ...
          console.log(
            `Row ${
              i + 1
            }: Skipped - Consigned Date: ${consignedDateValue}, Handover Time: ${handoverTimeValue}`
          );
        }
        continue;
      }

      // Calculate time difference: P column - O column (in hours)
      const timeDiffMs = handoverTime.getTime() - consignedDate.getTime();
      ...
      if (i < 10) {
        ...
        console.log(
          `  Consigned Date: ${consignedDate.toISOString()}`
        );
        console.log(`  Handover Time: ${handoverTime.toISOString()}`);
  ...
  useEffect(() => {
    if (uploadedData && uploadedData.data && Array.isArray(uploadedData.data)) {
      console.log('DeliveryKPI received data, processing...');
    ...
    if (filters.sortField && filters.sortDirection) {
      filtered.sort((a, b) => {
        ...
        if (
          ...
          filters.sortField === 'handoverTime'
        ) {
              ...
              <tr style={{ backgroundColor: '#f8f9fa' }}>
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleSort(tableKey, 'handoverTime')}
                      style={{
                        background: 'none',
                        border: 'none',
                        cursor: 'pointer',
                    ...
                    >
                      {getSortIcon(tableKey, 'handoverTime')}
                    </button>
                  </div>
                </th>
                <th
            ...
            <tbody>
              {paginatedData.map((row, index) => (
                <tr key={index}>
                  ...
                  <td style={{ padding: '8px', border: '1px solid #dee2e6' }}>
                    {row.handoverTime.toLocaleString()}
                  </td>
                  <td
                    style={{
                      padding: '8px',
