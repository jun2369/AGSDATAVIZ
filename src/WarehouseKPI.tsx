...
const WarehouseKPI: React.FC<WarehouseKPIProps> = ({ uploadedData }) => {
  ...
  const exportToExcel = (data: ExcelRow[], filename: string) => {
    const exportData = data.map(row => ({
      ...
      'Custom Final Release Date': row.customReleaseDate.toLocaleString(),
      ...
      <div className="kpi-table-section" style={{ marginBottom: '30px' }}>
        ...
        >
          ...
          {showExportButton && paginatedData.length > 0 && (
            <button
              onClick={() => {
                const filename = tableKey === 'lessThanZero' ? 'Warehouse_Negative_Hours' : 'Warehouse_More_Than_72_Hours';
